...
const WORK_DURATION_SECONDS = 10; // for testing
const BREAK_DURATION_SECONDS = 10; // for testing
...
const Pomodoro = forwardRef<PomodoroHandle, PomodoroProps>(
  function Pomodoro({ task, onTimerStateChange }, ref) {
    ...
    function extractErrorMessage(
      ...
      return (
        errors._form?.[0] || Object.values(errors).flat()[0] || fallback
      );
    }
    ...
    async function handleStartButton(): Promise<void> {
      ...
      if (result.success) {
        ...
        alert(extractErrorMessage(result.errors, "セッションの開始に失敗しました"));
      }
    }
    ...
    return (
      <div className="flex flex-col rounded-lg border-white shadow-md shadow-gray-400 hover:bg-gray-700">
        <div className="p-3 border rounded-lg flex flex-col gap-3 bg-white">
          ...
          <div
            className={`p-3 flex flex-col gap-3 border border-white border-l-4 rounded-lg ${
              timerMode === "work"
          ...
          <div className="p-2 sm:p-3 flex flex-col gap-2 sm:gap-3">
            <div
              className={`flex mx-auto text-5xl font-extralight ${
                timerMode === "work"
                  ? "text-red-600"
                  : timerMode === "break"
                    ? "text-green-600"
