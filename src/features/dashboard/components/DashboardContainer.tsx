...
const DashboardContainer = ( { incompleteTasks }: { incompleteTasks: Task[] }  ) => {
  ...
  return (
    <>
      ...
      <div >
        <Link href="/tasks/new" 
              className="flex justify-center border rounded bg-red-800 
              font-semibold text-white shadow-md shadow-red-900/40 
              transition hover:-translate-y-0.5 hover:bg-red-700 border-red-800">
          タスク追加
        </Link>
      </div>
      <br></br>
      ...
      <div className="flex flex-col gap-4 mt-4">
        <h1 className="text-1.5xl font-bold">今日の​タスク</h1>
        {tasks.map((task) => (
          <div key={task.id} 
            className="p-3 border rounded shadow-md shadow-red-900/40 
              flex flex-col gap-3 bg-white/90"
            ...
            <div className="flex items-center w-full">
              ...
              <div className="flex-none ml-auto">
                <Link href={`/tasks/${task.id}/edit`} 
                      className="font-bold text-xs text-white px-2 py-1.5 bg-red-600 hover:underline border rounded p-1"
